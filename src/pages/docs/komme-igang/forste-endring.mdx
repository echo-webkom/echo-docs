...
git clone git@github.com:echo-webkom/echo-web-mono
```
...
<Callout>`code .` er en kommando som åpner mappen i VSCode.</Callout>
...
For å kunne kjøre koden må du starte databasen. Dette gjør du ved å bruke kommandoen `pnpm db:up`. Dette krever at du har Docker installert på maskinen din.

```sh filename="Terminal"
pnpm db:up
```

Etter du har startet databasen må du oppdatere tabellene i databasen.
...
Denne kommandoen vil starte nettsiden på [http://localhost:3000](http://localhost:3000), dokumentasjonen på [http://localhost:3001](http://localhost:3001), Sanity Studio på [http://localhost:3333](http://localhost:3333) og [Drizzle Studio](https://orm.drizzle.team/drizzle-studio/overview) på [http://localhost:4000](http://localhost:4000).
...
<Callout>`origin` er i dette tilfellet GitHub repositoryet.</Callout>
